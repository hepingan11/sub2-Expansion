...
func (app *App) migrate() error {
	sqlStatements := []string{
		...
		`CREATE TABLE IF NOT EXISTS lottery_draws (
			...
			min_recharge DECIMAL(10, 2) NOT NULL,
			eligible_count INT NOT NULL,
			winner_count INT NOT NULL,
			prize_amount DECIMAL(10, 2) NOT NULL DEFAULT 0,
			created_at TIMESTAMPTZ NOT NULL
		)`,
		`ALTER TABLE lottery_draws ADD COLUMN IF NOT EXISTS prize_amount DECIMAL(10, 2) NOT NULL DEFAULT 0`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uk_lottery_draws_request_id ON lottery_draws (request_id)`,
		...
		`CREATE TABLE IF NOT EXISTS lottery_entries (
			...
			award_status VARCHAR(20) NOT NULL DEFAULT 'NONE',
			award_error TEXT NOT NULL DEFAULT '',
			awarded_at TIMESTAMPTZ NULL,
			created_at TIMESTAMPTZ NOT NULL,
			updated_at TIMESTAMPTZ NOT NULL
		)`,
		`ALTER TABLE lottery_entries ADD COLUMN IF NOT EXISTS award_status VARCHAR(20) NOT NULL DEFAULT 'NONE'`,
		`ALTER TABLE lottery_entries ADD COLUMN IF NOT EXISTS award_error TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE lottery_entries ADD COLUMN IF NOT EXISTS awarded_at TIMESTAMPTZ NULL`,
		`ALTER TABLE lottery_entries ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uk_lottery_entries_draw_user ON lottery_entries (draw_id, user_id)`,
		`CREATE INDEX IF NOT EXISTS idx_lottery_entries_winner ON lottery_entries (draw_id, winner)`,
		`CREATE INDEX IF NOT EXISTS idx_lottery_entries_award_status ON lottery_entries (draw_id, award_status)`,
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS uk_invitation_codes_user ON invitation_codes (user_id)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uk_invitation_codes_code ON invitation_codes (code)`,
		`CREATE TABLE IF NOT EXISTS invitation_bindings (
			...
			platform VARCHAR(40) NOT NULL,
			external_user_id VARCHAR(128) NOT NULL,
			invitee_created_at TIMESTAMPTZ NOT NULL,
			reward_amount DECIMAL(10, 2) NOT NULL,
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS uk_invitation_bindings_invitee ON invitation_bindings (invitee_user_id)`,
