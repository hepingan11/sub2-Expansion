...
3. 机器人使用新用户的社交平台 ID 和邀请码请求绑定链接。
4. 新用户必须自行打开该链接并登录，完成平台账号和邀请码绑定。
5. 后端校验邀请码、时间门槛、非自邀、未被其他邀请人绑定等条件后，向邀请人发放奖励。

绑定链接包含平台账号标识与邀请码，只能私发给对应新用户。邀请奖励仅以 `REWARDED` 状态为成功依据。
...
| `AUTH_SECRET` | 管理端令牌签名密钥，生产环境必须使用随机长字符串 |
...
- 部署前替换示例中的 `POSTGRES_PASSWORD`、`ADMIN_PASSWORD` 和 `AUTH_SECRET`。
