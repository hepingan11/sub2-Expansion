export default {
    accounts: {
      ...
      video: {
        format: '接口格式',
        baseUrlHint: '填写 API 根地址或完整的 OpenAI Videos 端点；默认请求路径为 /v1/videos。',
        apiKeyHint: '以 Authorization Bearer Token 发送给视频上游。',
        tokenHint: '以原始 Authorization Token 发送给 ComfyUI 上游，不添加 Bearer 前缀。',
        baseUrlRequired: '请输入 Video API Base URL',
        comfyBaseUrlHint: '填写 ComfyUI 服务根地址，例如 https://autodl.art。',
        workflowId: 'Workflow ID',
        workflowIdHint: '模型映射的目标值可覆盖此默认 Workflow ID。',
        workflowIdRequired: '请输入 ComfyUI Workflow ID',
        completeEndpoint: 'Base URL 已是完整视频端点',
        completeEndpointHint: '当地址已经包含供应商的视频提交路径时开启。',
        openAITestHint: '提交 POST /v1/videos，并轮询 GET /v1/videos/{id} 直到任务完成。',
        comfyTestHint: '提交 ComfyUI workflow 任务，并轮询 result/{task_id} 直到任务完成。',
        duration: '时长（秒）',
        aspectRatio: '画面比例',
        resolution: '分辨率',
        images: '参考图片 URL（可选）',
        imagesPlaceholder: '每行一个 HTTP/HTTPS URL',
        imagesHint: '最多 7 个 URL，不能与源视频 URL 同时使用。',
        sourceVideoUrl: '源视频 URL（可选）',
        sourceVideoHint: '用于视频生视频，不能与参考图片同时使用。'
      },
      anthropic: {
