...
export const BILLING_MODE_VIDEO_PER_SECOND = 'video_per_second'
export const BILLING_MODE_VIDEO_PER_REQUEST = 'video_per_request'

export function isVideoBillingMode(mode: string | null | undefined): boolean {
  return mode === BILLING_MODE_VIDEO || mode === BILLING_MODE_VIDEO_PER_SECOND || mode === BILLING_MODE_VIDEO_PER_REQUEST
}
...
export function getBillingModeLabel(mode: string | null | undefined, t: (key: string) => string): string {
  switch (mode) {
    ...
    case BILLING_MODE_VIDEO_PER_SECOND:
    case BILLING_MODE_VIDEO_PER_REQUEST: return t('admin.usage.billingModeVideo')
...
export function getBillingModeBadgeClass(mode: string | null | undefined): string {
  switch (mode) {
    ...
    case BILLING_MODE_VIDEO:
    case BILLING_MODE_VIDEO_PER_SECOND:
    case BILLING_MODE_VIDEO_PER_REQUEST: return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300'
...
export function isImageUsage(row: Pick<ImageBillingRow, 'image_count' | 'billing_mode'> | null | undefined): boolean {
  return (row?.image_count ?? 0) > 0 && row?.billing_mode !== BILLING_MODE_TOKEN && !isVideoBillingMode(row?.billing_mode)
}
...
export function getDisplayBillingMode(row: Pick<ImageBillingRow, 'billing_mode' | 'image_count'> | null | undefined): string | null | undefined {
  ...
  if (isVideoBillingMode(row?.billing_mode) || row?.billing_mode === BILLING_MODE_TOKEN) {
