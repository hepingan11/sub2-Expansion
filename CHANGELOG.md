...
- [新增] 渠道改为选择当前账号在 sub2api 中实际创建的分组，按精确分组获取 API Key 与模型列表，并支持 Grok 及仅文本的 Claude 分组。

- [修复] sub2api 登录改由同源服务端代理完成，并修复未登录访问业务页面时无法跳转登录页的问题。

- [新增] 接入 sub2api 账号登录、本地文件夹工作区授权及按用户隔离画布数据。
