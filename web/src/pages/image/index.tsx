...
export default function ImagePage() {
    ...
    const deleteSelectedLogs = () => {
        ...
        void Promise.all([deleteStoredImages(imageKeys), ...selectedLogIds.map((id) => logStore.removeItem(`${activeStorageScope()}:${id}`))]).then(refreshLogs);
    ...
    const saveLog = (log: GenerationLog) => {
        void logStore.setItem(`${activeStorageScope()}:${log.id}`, serializeLog(log)).then(refreshLogs);
    };
...
async function readStoredLogs() {
    ...
    try {
        ...
        await logStore.iterate<GenerationLog, void>((value, key) => {
            if (!key.startsWith(`${activeStorageScope()}:`)) return;
            values.push(value);
        });
