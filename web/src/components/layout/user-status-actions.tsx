...
export function UserStatusActions({ showConfig = true, variant = "default", onOpenShortcuts, onOpenPlugins }: UserStatusActionsProps) {
    ...
    return (
        <div className="inline-flex shrink-0 items-center gap-1">
            ...
            {user ? <button type="button" className="ml-1 inline-flex max-w-32 items-center gap-1.5 truncate border-l border-stone-200 pl-2 text-xs text-stone-500 hover:text-stone-950 dark:border-stone-700 dark:hover:text-white" onClick={() => void logout()} title="退出登录"><span className="truncate">{user.displayName}</span><LogOut className="size-3.5 shrink-0" /></button> : null}
            {onOpenShortcuts ? (
