...
async function storeImage(blob: Blob, options?: ImageReadOptions): Promise<UploadedImage> {
    const storageKey = `${activeStorageScope()}:image:${nanoid()}`;
...
export function collectImageStorageKeys(value: unknown, keys = new Set<string>()) {
    ...
    if ("storageKey" in value && typeof value.storageKey === "string" && (value.storageKey.startsWith("image:") || value.storageKey.includes(":image:"))) keys.add(value.storageKey);
