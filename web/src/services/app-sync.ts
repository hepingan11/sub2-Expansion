...
async function downloadMissingFiles<T>(config: WebdavSyncConfig, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    for (const storageKey of storageKeys) {
        const localBlob = (storageKey.startsWith("image:") || storageKey.includes(":image:")) ? await getImageBlob(storageKey) : await getMediaBlob(storageKey);
        scanned += 1;
        if (localBlob) {
    ...
    await runWithConcurrency(tasks, FILE_CONCURRENCY, async (remoteFile) => {
        ...
        await ((remoteFile.storageKey.startsWith("image:") || remoteFile.storageKey.includes(":image:")) ? setImageBlob(remoteFile.storageKey, typedBlob) : setMediaBlob(remoteFile.storageKey, typedBlob));
        downloaded += 1;
...
async function uploadChangedFiles<T>(config: WebdavSyncConfig, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    let scanned = 0;
    for (const storageKey of storageKeys) {
        const blob = (storageKey.startsWith("image:") || storageKey.includes(":image:")) ? await getImageBlob(storageKey) : await getMediaBlob(storageKey);
        const remoteFile = remoteFileMap.get(storageKey);
        if (!blob) {
...
async function readStoredLogs(store: LogStore) {
    ...
    await store.iterate<StoredLog, void>((value, key) => {
        if (key.startsWith(`${activeStorageScope()}:`) && value && typeof value === "object") logs.push(value);
    });
    return logs;
}
...
async function replaceStoredLogs(store: LogStore, logs: StoredLog[]) {
    ...
    await store.iterate((_value, key) => { if (key.startsWith(`${activeStorageScope()}:`)) ownKeys.push(key); });
    await Promise.all(ownKeys.map((key) => store.removeItem(key)));
    await runWithConcurrency(logs, FILE_CONCURRENCY, async (log) => {
        const id = getStringField(log, "id");
        if (id) await store.setItem(`${activeStorageScope()}:${id}`, log);
    });
}
...
function fileExtension(mimeType: string, storageKey: string) {
    ...
    return (storageKey.startsWith("image:") || storageKey.includes(":image:")) ? "png" : "bin";
}
