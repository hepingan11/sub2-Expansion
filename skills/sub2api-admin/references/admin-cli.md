...
后台鉴权优先使用 `SUB2API_ADMIN_API_KEY` 发送 `x-api-key`。未设置时，CLI 使用 `SUB2API_ADMIN_EMAIL` 和 `SUB2API_ADMIN_PASSWORD` 调用登录接口，拿到 `data.access_token` 后仅在当前进程内作为 bearer token 使用。如果返回 `INVALID_ADMIN_KEY`，重新生成管理员 API Key。
...
不要把密钥、导出账号凭据、OAuth token 或完整 access token 写入聊天消息。优先通过环境变量或本地文件传入。
...
```bash
node scripts/sub2api-admin.js settings admin-api-key status
node scripts/sub2api-admin.js settings admin-api-key regenerate
node scripts/sub2api-admin.js settings admin-api-key delete
```
