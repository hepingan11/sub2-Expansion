...
async function loginAccessTokenOnce() {
  if (loginAccessToken) return loginAccessToken;
  if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
    throw new Error("Missing SUB2API_ADMIN_API_KEY or SUB2API_ADMIN_EMAIL/SUB2API_ADMIN_PASSWORD");
  }
  ...
  if (!res.ok || (data && data.code !== undefined && data.code !== 0 && data.code !== "0")) {
    ...
    throw new Error(`POST /api/v1/auth/login failed: ${detail}`);
  }
  const token = data && data.data && data.data.access_token;
  if (!token) throw new Error("POST /api/v1/auth/login failed: missing data.access_token");
...
async function authHeaders() {
  ...
  return { Authorization: `Bearer ${await loginAccessTokenOnce()}` };
}
...
async function commandSettings(args) {
  const area = args.positional[1];
  const action = args.positional[2];
  if (area === "admin-api-key") {
    if (action === "status") {
      printJson(await adminRequest("GET", "/admin/settings/admin-api-key"));
      return;
    }
    if (action === "regenerate") {
      printJson(await adminRequest("POST", "/admin/settings/admin-api-key/regenerate"));
      return;
    }
    if (action === "delete") {
      printJson(await adminRequest("DELETE", "/admin/settings/admin-api-key"));
      return;
    }
    throw new Error("settings admin-api-key requires status|regenerate|delete");
  }
  throw new Error(`unknown settings area: ${area || "(missing)"}`);
}
